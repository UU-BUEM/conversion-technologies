...
- [config] `config/config.py`'s `ExportConfig`/`load_export_config` is wired
  but not yet consumed by the CLI (`export`/`export-all` take flags directly).
  Wire a `--config path.json` flag into `cli.py` if batch scenario files
  become a real workflow need.
